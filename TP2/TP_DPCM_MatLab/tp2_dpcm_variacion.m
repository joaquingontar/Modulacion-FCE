...
% Params
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
A=10;          % A: amplitud de la Sx Senoidal, 5 a 20, default 10
...
P=2*pi;
t=(0 : P/M : P*ciclos);       
%a=A*sin(t).*exp(-t) + 0.5*exp(t-8); % version exponecial
a=A*sin(t);                                     % version senoidal
n=length(a);
x(1:n)=a;
...
for k=2:n
    ...
    % Evita que delta k se incremente o atenue mas de un cierto valor.
    % Sin este bloque delta_k puede crecer o disminuir sin limite.
    % treshold
    if delta_k > 5
        delta_k = 5;
    elseif delta_k < 0.1
    ...
    if (d(k-1)  == -1 && d(k) == 1)                % modo granular >> prox esc +50%
        xhat(k+1) = xhat(k) + atenuacion * delta_k;	  
